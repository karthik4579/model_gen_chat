...
def generate_prompts(dataset_goal, seed_data_val="", dataset_type_val="", should_search_val="true"):
    ...
    if seed_data_val == "":
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
            futures = {executor.submit(process_batch, i): i for i in range(start_range,end_range)}
...
def generate_data(dataset_type_val, dataset_goal):
    ...
    if dataset_type_val == "text_only":
      ...
      'order': ["Groq"]
            ...
            try:
                response = make_sequential_request(endpoint, headers, instance, retry_count=10)
                if not isinstance(response, dict) or 'choices' not in response:
        ...
        for prompt, response in current_dataset.items():
            ...
            if 70 <= current_similarity_score <= similarity_threshold:
                ...
                if eval_result >= 4:
                    ...
                    current_temp_eval_result = eval_result
                    while True:
                        if current_temp_eval_result <= 4:
                            break
                        else:
                            ...
                            rewrite_input = f"""
                                            ...
                                            current_score : {current_temp_eval_result}
                                            """
                            instance = {
                                    "model": main_model_name,
                                    "messages": [
                            ...
                            print(evals)
                            current_critique = evals["reasoning"]
                            current_temp_eval_result = evals["result"]
                    print("got outside the improvement loop ...")
                    current_dataset[current_prompt] = current_response
        
        dataset_path = f"{Path.cwd()}/generated_datasets/{uuid.uuid4().hex[:12]}"
        temp_final_dataset = list(current_dataset.items())
        train_test_size = int(len(temp_final_dataset) * 0.8)
        train_samples = temp_final_dataset[:train_test_size]
        test_samples = temp_final_dataset[train_test_size:]
        try:
